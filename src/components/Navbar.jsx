...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Navbar is visible if we're at the top, scrolling up, or hovering over it
      const visible = prevScrollPos > currentScrollPos || currentScrollPos < 10 || isHovering
  ...
  }, [prevScrollPos, isHovering]) // Add isHovering to dependency array
  ...
  return (
    <>
      ...
      >
        ...
        <div className="hidden md:flex space-x-16 text-center text-[15px] relative">
          ...
          >
            ...
            <div className="absolute left-1/2 -translate-x-1/2 mt-2 bg-black text-white p-4 rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 shadow-lg">
              <Link to="/product/moval" className="block py-2 px-4 rounded-md hover:bg-gray-700 transition-colors duration-300">
                Moval
              </Link>
              <Link to="/product/cars" className="block py-2 px-4 rounded-md hover:bg-gray-700 transition-colors duration-300">
                CARS
              </Link>
            </div>
          </div>
        ...
        {/* Desktop Connect Button - Responsive Placeholder */}
        {location.pathname !== "/contact" ? (
          ...
          <div className="hidden md:inline-block w-48"></div> // same width as button placeholder on desktop
        )}
        ...
        <div className="flex flex-col space-y-8 font-medium">
          ...
          <div className="relative group">
            ...
            <div className="mt-4 space-y-4">
              <Link to="/product/moval" className="block text-white text-1xl" onClick={toggleMobileMenu}>
                Moval
              </Link>
              <Link to="/product/cars" className="block text-white text-1xl" onClick={toggleMobileMenu}>
                CARS
              </Link>
            </div>
          </div>
          ...
          {location.pathname !== "/contact" ? (
            ...
            <div className="text-white text-3xl invisible">Placeholder</div> // Keeps space in mobile menu too
          )}
        </div>
      </div>
    </>
