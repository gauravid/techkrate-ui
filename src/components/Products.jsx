...
//       to={to}
//       ref={cardRef}
//       className="relative group overflow-hidden rounded-2xl aspect-[3/4] transition-all duration-300"
//     >
//       {/* Background Image */}
//       <div 
...
const ProductCard = ({ name, logo, description, background, to }) => {
  ...
  return (
    <Link 
      to={to}
      ref={cardRef}
      className="relative group overflow-hidden rounded-2xl aspect-[3/4] transition-all duration-200"
    >
      {/* Background Image */}
      <div 
        className="absolute inset-0 w-full h-full bg-cover bg-center transition-transform duration-500 group-hover:scale-110"
      ...
      <div className="relative h-full flex flex-col items-center justify-between p-4 text-white">
        ...
        {/* Center Logo */}
        <div className="w-full h-full rounded-2xl p-2 flex items-center justify-center transform transition-transform duration-300 group-hover:scale-110">
          <img 
            src={logo} 
            alt={`${name} logo`} 
            className="w-3/4 h-full object-contain"
        ...
        {/* Bottom Description */}
        <p className="text-sm md:text-lg text-center max-w-sm font-medium font-Helix">
          {description}
        </p>
      </div>
    </Link>
...
const Products = () => {
  ...
  return (
    <div className="w-full flex flex-col items-center pb-8 px-4">
      <h1 className="text-3xl sm:text-4xl md:text-6xl font-bold mb-8 sm:mb-16 text-center text-white font-Helix">
        Products
      </h1>
      <div 
        ref={containerRef}
